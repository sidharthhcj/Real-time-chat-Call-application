...
export default function Chat() {
  ...
  useEffect(() => {
    ...
    s.on("receive-message", (data) => {
      setMessagesByRoom((prev) => ({
        ...
        [data.roomId]: [
          ...(prev[data.roomId] || []),
          { _id: Date.now(), sender: data.sender, message: data.message },
        ],
      }));

      // 🔥 AI reply trigger
      fetchSmartReplies(data.message);
    });
  ...
  const sendMessage = () => {
    ...
    setMessagesByRoom((prev) => ({
      ...
      [currentRoom]: [
        ...(prev[currentRoom] || []),
        { _id: Date.now(), sender: "me", message },
      ],
    }));
  ...
  return (
    <div className="flex h-screen bg-slate-900 text-white">
      ...
      <div className="w-1/4 border-r border-slate-700 flex flex-col">
        ...
        <input
          className="m-2 p-2 bg-slate-800 rounded"
          placeholder="Search user"
          value={search}
          onChange={(e) => setSearch(e.target.value)}
        />
      ...
      {/* RIGHT */}
      <div className="w-3/4 flex flex-col">
        <div className="p-4 bg-slate-950 flex justify-between">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
          ...
          {messages.map((m) => (
            ...
            >
              <div className="bg-slate-700 px-3 py-2 rounded max-w-xs">
                {m.message}
              </div>
            </div>
          ))}
        ...
        {/* 🔥 AI SMART REPLIES */}
        {aiReplies.length > 0 && (
          <div className="p-2 flex gap-2 flex-wrap bg-slate-800">
            {aiReplies.map((r, i) => (
              <button
                key={i}
                onClick={() => {
        ...
        {/* INPUT */}
        {selectedUser && (
          <div className="p-3 flex gap-2 bg-slate-950">
            <input
